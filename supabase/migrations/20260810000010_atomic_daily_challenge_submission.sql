-- Migration ID: 20260810000010_atomic_daily_challenge_submission.sql
-- Atomic Daily Challenge Submission RPC: quiz_attempt + answers + completion in one transaction
-- Database-authoritative scoring. Revokes solo complete_daily_challenge from all client roles.
...
BEGIN
  ...
  -- 9. Generate attempt ID
  v_attempt_id := 'att_dc_' || extract(epoch from v_completed_at)::BIGINT::TEXT || '_' ||
                  substr(md5(random()::TEXT), 1, 8);

  -- 10. Database-authoritative scoring + insert quiz_answers
  ...
  -- 12. Insert daily_challenge_completions
...
-- 2. REVOKE complete_daily_challenge from all client roles (still exists for DB integrity, just not callable by clients)
REVOKE EXECUTE ON FUNCTION public.complete_daily_challenge(UUID, TEXT) FROM PUBLIC, anon, authenticated;
